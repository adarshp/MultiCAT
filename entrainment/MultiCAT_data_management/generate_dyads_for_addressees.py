...
OPENSMILE_CONFIG_BASELINE = (
    parent_dir + "/scripts_and_config/emobase2010_haoqi_revised.conf"
)
...
def loop_through_data(combined_transcript_dict, save_dir):
    ...
    for fpath, combined_transcript in combined_transcript_dict.items():
        ...
        combined_transcript = combined_transcript.sort_values(
        ...
        for speaker_pair in speaker_pairs:
                ...
                if i < len(combined_transcript) - 1:
                    ...
                    if row.speaker != next_row.speaker:
                        ...
                        if extract_me:
                            ...
                            this_row_feats, this_row_copy = (
                                run_opensmile_over_utterance(row, fpath)
                            )
                            ## extract for following row
                            next_row_feats, next_row_copy = (
                                run_opensmile_over_utterance(next_row, fpath)
                            )
            ...
            if np.shape(features_for_normalizing)[0] <= 0:
                ...
                all_raw_norm_feat, all_norm_feat_dim = calc_opensmile_feats(
                    features_for_normalizing
                )
                ...
                for i in range(len(all_features_for_this_list)):
                    if i % 2 == 0:
                        ...
                        whole_func_feat = np.hstack(
                            (whole_func_feat1, whole_func_feat2)
                        )
...
def id_whether_to_extract(row, following_row, speaker_pair):
    if row.speaker in speaker_pair:
        ...
        print(
            f"speaker: {speaker}, intended listener: {listener}, addressee: {row.addressee}"
        )

        ## check if conditions are met: a given utterance and the successive turn
        ## have the same interlocutors:
        if (
            row.addressee == listener
            and following_row.speaker == listener
            and following_row.addressee == speaker
        ):
            return True
...
if __name__ == "__main__":
    ...
    if not output_dir.exists():
        ...
        print(
            f"Could not find specified output directory {output_dir}. Creating directory..."
        )
    else:
