...
def make_argument_parser():
    parser = argparse.ArgumentParser(
        description="Optional arguments for running train.py"
    )
    parser.add_argument(
        "--model_name",
        type=str,
...
def train(each_epoch, model, train_loader, optimizer, cuda):
    ...
    print(("====> Epoch: {} Average loss: {:.4f}".format(each_epoch, train_loss)))
...
if __name__ == "__main__":
    ...
    if os.path.isdir(model_directory_path):
        print(
            f"The directory for storing the trained model '{model_directory_path}' exists. Continue..."
        )
    else:
        print(
            f"The directory for storing the trained model '{model_directory_path}' does not exist. Creating directory..."
        )
    ...
    if os.path.isdir(args.h5_directory):
        print(
            f"The directory with training files '{args.h5_directory}' exists. Continue..."
        )
    else:
        print(
            f"The directory with training files '{args.h5_directory}' does not exist. Recheck dir path."
        )
        sys.exit(1)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        vloss = validate(
            model=baseline_model, val_loader=baseline_val_loader, cuda=args.cuda
        )
        Tloss.append(tloss)
        Vloss.append(vloss)
        if vloss < best_loss:
