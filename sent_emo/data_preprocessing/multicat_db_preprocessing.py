...
print(
    f"Training data: {len(train_data)} records, Validation data: {len(dev_data)} records, Test data: {len(test_data)} records"
)
...
def extract_ys_data(dataset):
    ...
    """
    return [
        {"audio_id": item["message_id"], "ys": [item["sentiment"], item["emotion"]]}
        for item in dataset
    ]
...
sentiment_weights = compute_class_weight(
    class_weight="balanced", classes=np.unique(train_sentiments), y=train_sentiments
)

# Compute class weights for emotion
emotion_weights = compute_class_weight(
    class_weight="balanced", classes=np.unique(train_emotions), y=train_emotions
)
