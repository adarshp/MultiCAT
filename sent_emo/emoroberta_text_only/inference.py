# get results from EmoRoBERTa and convert to sentiment and Ekman emotions
import shutil
import sys
import os
...
from baselines.sent_emo.emoroberta_text_only.model import EmoRoBERTa
...
def predict(
    ...
    # convert this output into sentiment and emotion
    print(batch_pred)
    exit()
...
def finetune(dataset, device, output_path, config):
    ...
    model = EmoRoBERTa(model_params, device)
...
if __name__ == "__main__":
    ...
    import baselines.sent_emo.emoroberta_text_only.emoroberta_config as config
    ...
    with open(os.path.join(output_path, "all_trials.csv"), "a") as writef:
        writef.write(
            f"{date.today()},EmoRoBERTa,{config.model_params.model},{config.model_params.lr},"
        )
        writef.write(
